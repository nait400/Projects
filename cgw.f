      program main
      integer i,j, ierr, t
      real*8 Ham(6,6),g, fv1(6), fv2(6), fv3(6), matrix(6,6)
      real*8 wreal(6), wimg(6), zreal(6,6), zimg(6,6)
c
      open(1,FILE='plot.txt')
c
c      do k=0,100
c
c      g = -20.+0.4*k
...
c
      call cg(6, 6, Ham, matrix, wreal, wimg, 1, zreal, zimg,
     1         fv1, fv2, fv3, ierr)
c      
      j=1
      do k=1,6
         if (wreal(k) .lt. wreal(j)) then
            j=k
         end if
      end do
      print *, wreal(j),zreal(j,1),zreal(j,2),zreal(j,3),zreal(j,4),
     &       zreal(j,5),zreal(j,6)
c
c      write(1,*) Ham(1,1),Ham(2,2),Ham(3,3),Ham(4,4),Ham(6,6)
c      do i=1,6
c      write(1,*)g, wreal(1),wreal(2),wreal(3),wreal(4),wreal(5),
c     &       wreal(6),2.-g,4.-g,6.-g,8.-g,10.-g     
c      end do
c
c      end do
c      close(1)
